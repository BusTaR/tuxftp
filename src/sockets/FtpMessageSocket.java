...
public class FtpMessageSocket {
	...
	public static Socket ftpMsgSocket = null;  // create an Socket
	...
	public Socket startMessageSocket(String address, int port) {
	   	   try {	   		   
	   		   		// get Socket
	   		   		ftpMsgSocket = new Socket(address, port);
	...
	public static synchronized InputStream getSocketInput() {
	 try {
		InputStream instream = ftpMsgSocket.getInputStream();
		return instream;
	} catch (IOException e) {
	...
	public static synchronized OutputStream getSocketOutput() {
		...
		try {
			outstream = ftpMsgSocket.getOutputStream();
			return outstream;
		} catch (IOException e) {
	...
	// get output Stream        
		try {
			out = new PrintWriter(ftpMsgSocket.getOutputStream(), true);
			return out;
		} catch (IOException e) {
  ...
  return null;
	...
	public static synchronized BufferedReader input() {
	   // get input Stream
		try {
			inputstreamreader = new InputStreamReader(ftpMsgSocket.getInputStream());
